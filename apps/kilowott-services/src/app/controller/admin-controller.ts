...
import Config from '../../../../../libs/shared/dotenv';
import User from '../../../../../libs/shared/datamodels/User';
import Message from '../../../../../libs/shared/message';
import { sign} from '../../../../../libs/shared/utils/jwt.utils';
import getPaginationDetails from '../../../../../libs/shared/pagination';
import * as nodemailer from 'nodemailer';

export default class onBoardAdminController {
  //Onboarding the Admin
  static adminSignUp(requestObject) {
          ...
          .then(async (user) => {
            if (user) {
              ...
              this.comparePassword(password, user.password, (err, isMatch) => {
                if (isMatch) {
                  sign(user.email, user.roleName).then((token) => {
                    const responseData = {
                      id: user._id,
  ...
  static updateUser(requestObject) {
    return new Promise((resolve, reject) => {
      try {
  ...
  static changePassowrd(requestObject) {
    return new Promise((resolve, reject) => {
      try {
  ...
  static deleteUser(requestObject) {
    return new Promise((resolve, reject) => {
      try {
  ...
  static updateUserStatus(requestObject) {
    return new Promise((resolve, reject) => {
      try {
        const { userId } = requestObject.params;
        ...
        User.findOne({ _id: userId })
          .then(async (userExists) => {
            if (userExists) {
              ...
              userExists.save().then((disableUser) => {
                if (disableUser) {
                  return resolve({
                    status: 200,
                    success: true,
  ...
  static comparePassword(candidatePass, userPass, cb) {
